...
function loadEndpoints() {
  try {
    if (fs.existsSync(ENDPOINTS_FILE)) {
      const data = JSON.parse(fs.readFileSync(ENDPOINTS_FILE, 'utf-8'));
      if (Array.isArray(data) && data.length > 0) {
        return data;
      }
    }
  } catch (err) {
    console.error('Error loading custom endpoints:', err.message);
  }
  saveEndpoints(DEFAULT_CHANNELS);
  return DEFAULT_CHANNELS;
}

function saveEndpoints(channels) {
  try {
    fs.writeFileSync(ENDPOINTS_FILE, JSON.stringify(channels, null, 2), 'utf-8');
  } catch (err) {
    console.error('Error saving endpoints:', err.message);
  }
}
